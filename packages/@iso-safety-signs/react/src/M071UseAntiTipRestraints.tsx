// THIS FILE IS AUTO-GENERATED. DO NOT EDIT MANUALLY.
// Run 'yarn generate' to regenerate.

import * as React from 'react';
import type { ParsedSvg } from '@iso-safety-signs/core';
import type { SignProps } from './SignProps';
import { SignSvg } from './SignSvg';

const _svg: ParsedSvg = {
  attrs: "xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 70.115 70.115\"",
  body: "<path fill=\"#005387\" d=\"M10.583 64.69c0-19.36 15.695-35.057 35.057-35.057S80.698 45.33 80.698 64.691 65.003 99.748 45.64 99.748c-19.362 0-35.057-15.694-35.057-35.057\" style=\"display:inline;stroke-width:.264583\" transform=\"translate(-10.583 -29.633)\"/><path d=\"M59.65 44.066v5.18h4.065v3.328H59.65v30.34H58.4v-30.34h-1.626v3.925H50.77v1.988H37.935v24.427H25.941v2.401H65.34v-41.25ZM39.41 45.64l-1.05 1.818 4.103 2.37H28.224v2.1h14.35l-4.107 2.37 1.05 1.819 9.02-5.207zm17.365 1.647v1.959H58.4v-1.96zm-1.205 1.038a2.575 2.575 0 0 0 0 5.15v-1.763h7.403v-1.624H55.57ZM41.617 80.437h13.101l1.574 2.477H40.044z\" style=\"color:#000;fill:#fff;fill-opacity:1;-inkscape-stroke:none;paint-order:stroke fill markers\" transform=\"translate(-10.583 -29.633)\"/>",
  height: "265",
  width: "265",
};
const _title = "Use anti-tip restraints";
const _description = "Use anti-tip restraints";

export const M071UseAntiTipRestraints = React.memo<SignProps>((props) => (
  <SignSvg {...props} defaultDescription={_description} defaultTitle={_title} svg={_svg} />
));
M071UseAntiTipRestraints.displayName = 'M071UseAntiTipRestraints';
