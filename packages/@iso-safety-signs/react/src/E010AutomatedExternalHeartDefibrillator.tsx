// THIS FILE IS AUTO-GENERATED. DO NOT EDIT MANUALLY.
// Run 'yarn generate' to regenerate.

import * as React from 'react';
import type { ParsedSvg } from '@iso-safety-signs/core';
import type { SignProps } from './SignProps';
import { SignSvg } from './SignSvg';

const _svg: ParsedSvg = {
  attrs: "xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 200 200\"",
  body: "<path d=\"M0 0h200v200H0z\" style=\"fill:#fff\"/><path d=\"M5 5h190v190H5z\" style=\"fill:#237f52\"/><path d=\"M132.73 36.116h47.168v15.723H132.73zm15.723-15.723h15.723v47.168h-15.723zM56.258 65.861c-18.603.194-35.268 18.7-30.23 43.116 3.779 18.7 23.641 42.922 58.231 66.37 34.59-23.544 54.452-47.767 58.231-66.37 5.038-24.513-11.53-42.922-30.23-43.116-9.592-.194-21.607 4.941-28.001 17.15-6.395-12.208-18.409-17.343-28.001-17.15M99.18 80.685l-16.278 33.427h20.638l-36.14 42.05 13.952-32.943H61.296z\" style=\"fill:#fff\"/>",
  height: "400",
  width: "400",
};
const _title = "Automated external heart defibrillator";
const _description = "Automated external heart defibrillator";

export const E010AutomatedExternalHeartDefibrillator = React.memo<SignProps>((props) => (
  <SignSvg {...props} defaultDescription={_description} defaultTitle={_title} svg={_svg} />
));
E010AutomatedExternalHeartDefibrillator.displayName = 'E010AutomatedExternalHeartDefibrillator';
