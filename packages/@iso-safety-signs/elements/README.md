# @iso-safety-signs/elements

Accessible Web Components (Custom Elements) for all **ISO 7010** workplace safety signs — framework-agnostic, zero runtime dependencies.

[![npm](https://img.shields.io/npm/v/@iso-safety-signs/elements)](https://www.npmjs.com/package/@iso-safety-signs/elements)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://github.com/karlnorling/iso-safety-signs/blob/main/LICENSE)

---

## Installation

```sh
npm install @iso-safety-signs/elements
# or
yarn add @iso-safety-signs/elements
```

---

## Usage

### Register all elements

```ts
import { defineCustomElements } from '@iso-safety-signs/elements';
defineCustomElements();
```

Then use the elements in HTML:

```html
<!-- Generic element — render any sign by ID -->
<iso-sign sign-id="e001-emergency-exit-left-hand" width="64"></iso-sign>

<!-- Named elements (generated by yarn generate) -->
<iso-e001-emergency-exit-left-hand width="64"></iso-e001-emergency-exit-left-hand>
<iso-w021-flammable-material width="64"></iso-w021-flammable-material>
```

### Custom prefix

Avoid tag-name conflicts with other libraries by passing a custom prefix:

```ts
defineCustomElements('safety');
// Registers: safety-sign, safety-e001-emergency-exit-left-hand, safety-w021-flammable-material, …
```

---

## Attributes

All elements and `<iso-sign>` accept the same attributes:

| Attribute     | Default              | Description                                                              |
| ------------- | -------------------- | ------------------------------------------------------------------------ |
| `sign-id`     | —                    | Sign slug ID (`<iso-sign>` only), e.g. `"e001-emergency-exit-left-hand"` |
| `title`       | Sign name            | Accessible title (`<title>` inside SVG)                                  |
| `description` | Sign description     | Accessible description (`<desc>` inside SVG)                             |
| `aria-label`  | —                    | Labels the `<svg>` directly instead of via title/description             |
| `width`       | SVG intrinsic width  | Forwarded to the `<svg>` width attribute                                 |
| `height`      | SVG intrinsic height | Forwarded to the `<svg>` height attribute                                |

All attributes are reactive — changing them re-renders the SVG.

---

## Framework usage

### React

```tsx
// React treats unknown elements as HTML. Add a JSX type declaration if needed.
declare namespace JSX {
  interface IntrinsicElements {
    'iso-sign': React.HTMLAttributes<HTMLElement> & {
      'sign-id'?: string;
      width?: string | number;
      height?: string | number;
    };
  }
}

<iso-sign sign-id="e001-emergency-exit-left-hand" width="64" />;
```

### Vue 3

```vue
<template>
  <iso-sign sign-id="e001-emergency-exit-left-hand" width="64" />
</template>
```

---

## Accessibility

Each element renders with `role="img"` and `aria-labelledby` pointing to a `<title>` and `<desc>` injected inside the SVG. An `aria-label` attribute on the element labels the `<svg>` directly instead. Ids are unique per element, so the same sign can appear any number of times on a page. The host element gets `display: contents` so it is transparent to layout.

---

## License

MIT © [Karl Norling](https://github.com/karlnorling)
